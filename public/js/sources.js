...
$(document).ready(()=>{
	...
	function getNews()
	{
		...
		let urls = [
			`${endPoint}?sources=bloomberg&apiKey=${apiKey} `,
			`${endPoint}?sources=bbc-news&apiKey=${apiKey} `,
			`${endPoint}?sources=cnn&apiKey=${apiKey} `	
		]; 

		let allResults = [];

		let count = urls.length-2;
		const get =(real)=>{
